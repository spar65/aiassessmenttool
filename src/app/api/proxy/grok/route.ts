/**
 * xAI Grok API Proxy - Conversational Mode Support
 *
 * Routes Grok API calls through the server to avoid CORS issues.
 * Supports both:
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[Grok Proxy] Error:", message);
...
async function handleConversationalMode(body: {
  ...
  if (!apiKey.startsWith("xai-")) {
    return NextResponse.json(
      { error: "Invalid Grok API key format. Key should start with 'xai-'" },
      { status: 400, headers: corsHeaders }
    );
  }
  ...
  try {
    ...
    if (!rawResponse) {
      return NextResponse.json(
        {
          error: "No response generated by Grok",
          type: "empty_response",
        },
        { status: 500, headers: corsHeaders }
      );
    ...
    return NextResponse.json(
      {
        ...
        model: data.model,
        usage: data.usage,
        contextSize: grokMessages.length,
      },
      { headers: corsHeaders }
    );
  } catch (error) {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      return NextResponse.json(
        { error: "Grok API request timed out after 30 seconds" },
